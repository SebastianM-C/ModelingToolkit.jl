...
function process_DynamicOptProblem(
    ...
    add_observed_bounds!(
        fullmodel, sys, pmap, tspan, tunable_params, bounds, observed_bounds_method, scales
    )
...
function add_observed_bounds!(
    ...
    for (var, (lo, hi)) in observed_bounds
        if method === :lift
            ...
            # A user-supplied scale overrides the nominal, mirroring the residual
            # scaling of the dynamics constraints.
            scale = get(scales, var, getnominal(var))
